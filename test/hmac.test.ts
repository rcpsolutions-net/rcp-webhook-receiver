...
import { computeHmac, safeCompare, verifySignature } from '../src/plugins/hmac';
...
test('computeHmac produces a consistent hex digest', (t) => {
  ...
  t.not(computeHmac('other-secret', 'hello world'), digest, 'different secret changes digest');
  t.not(computeHmac('secret', 'different payload'), digest, 'different payload changes digest');
  t.end();
});
...
test('safeCompare returns false for non-string inputs', (t) => {
  t.notOk(safeCompare(null as unknown as string, 'abc'));
  t.notOk(safeCompare('abc', undefined as unknown as string));
  t.end();
});
...
test('verifySignature accepts a sha256 prefixed signature', (t) => {
  const secret = 'my-secret';
  const payload = Buffer.from('{"event":"push"}');
